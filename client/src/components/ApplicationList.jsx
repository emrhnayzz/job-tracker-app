...
import { FaTrash, FaEdit, FaExternalLinkAlt, FaCalendarAlt } from 'react-icons/fa';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
      {applications.map((app) => (
        <div key={app.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-6 border border-gray-100 flex flex-col justify-between">
          ...
          <div className="flex justify-between items-center mt-4 border-t pt-4">
            {app.link ? (
              <a href={app.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 flex items-center gap-1 text-sm">
      ...
      {applications.length === 0 && (
        <p className="col-span-full text-center text-gray-500">No applications found. Start by adding one!</p>
      )}
    </div>
  );
};
